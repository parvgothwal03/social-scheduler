...
const getOrCreateZernioProfile = async (user:any) : Promise<string> => {
    try {
        ...
        body: {name : `${user.name || user.email}'s Profile`} as any});
        ...
        console.error("Error in getOrCreateZernioProfile:", error?.message || error);
        throw error;
    }
}
...
export const generateAuthUrl = async (req: AuthRequest, res: Response) : Promise<void> => {
    try {
        ...
        if(!authUrl) {
            throw new Error (`Failed to generate authorization URL. Full Response: ${JSON.stringify(data)}`);
        }
