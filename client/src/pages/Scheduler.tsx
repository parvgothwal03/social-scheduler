...
const Scheduler = () => {
    ...
    const handleSchedule = async(e: FormEvent<HTMLFormElement>) => {
        ...
        if(selectedPlatforms.includes('instagram') && !mediaFile) {
          toast.error("Instagram posts require media. Please upload an image or video.");  
          return;
    }
    ...
    try {
        await api.post("/api/posts", formData, {headers: {"Content-Type":
        "multipart/form-data"}});
        ...
        <div className='flex flex-col lg:flex-row gap-6 h-full'>
                     ...
                     {/* Media Upload */}
                        ...
                        {mediaFile ? (
                            ...
                            border-slate-200 bg-slate-50'>
                                ...
                                alt="Media preview" className='w-full h-40 object-cover'/> 
                                :
                                <video src={URL.createObjectURL(mediaFile)} className='w-full h-40
                                object-cover' controls/>}
                            ...
                            <div>
                               ...
                               hover:border-red-300 hover:bg-red-50/30 transition-all group'>
                                ...
                                transition-colors'>Click to upload media</span>
                                <input type="file" accept="image/*,video/*"
                                className='hidden' onChange={(e) => e.target.files?.[0] && setMediaFile(e.target.files[0]) } />
                               </label>
                            </div>
                        ) }
                     {/* Date & Time */}
                        ...
                        <div>
                            ...
                            <div className='relative'>
                                ...
                                outline-none' value={scheduledDate} onChange={(e) => setScheduledDate(e.target.value)}/>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className='relative'>
                                ...
                                bg-slate-50 border border-slate-200 rounded-lg text-slate-900 text-sm
                                outline-none' value={scheduledTime} onChange={(e) => setScheduledTime(e.target.value)}/>
                            </div>
                        </div>
                        </div>
                     {/* Submit */}
            ...
            <div className='flex-1 flex flex-col gap-6 min-w-0'>
                                ...
                                transition-colors'>
                                    <div className='flex items-center justify-between mb-2'>
                                        ...
                                        <div className='flex items-center gap-2'>
                                            ...
                                            <span className='text-sm text-slate-400'>{new Date(post.scheduledFor).toLocaleString()}</span>
                                        </div>
                                    </div>
                ...
                {/* Published Posts */}
                        ...
                        <h3 className='text-slate-900 text-sm'>Published Posts</h3>
                        <span className='ml-auto text-xs font-bold bg-zinc-100 text-zinc-700 px-2
                                ...
                                transition-colors'>
                                    <div className='flex items-center justify-between mb-2'>
                                        ...
                                        <div className='flex items-center gap-2'>
                                            ...
                                            text-slate-600 border border-slate-200 px-1.5 py-0.5 rounded-md
                                            font-semibold capitalize'>{post.mediaType}</span>}
                                            <span className='text-sm text-slate-400'>{new Date(post.updatedAt).toLocaleString()}</span>
