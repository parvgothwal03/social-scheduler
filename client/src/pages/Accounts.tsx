...
const Accounts = () => {
    ...
    useEffect(() => {
        ...
        if(connectedPlatform) {
            ...
            fetchAccounts(true, connectedPlatform, `${label}${handle} connected successfully!`)
        } else if(errorMsg) {
    ...
    const connectedIds = accounts.map((a) => a.platform === "instagram_business" ? "instagram" : a.platform)
        ...
        <div className='space-y-8 max-w-4xl'>
            ...
            justify-between gap-4 text-sm'>
                <div>
                    ...
                    <p className='text-slate-500 text-sm mt-0.5'>{accounts.length}
                    of {PLATFORMS.length} platforms connected</p>
                </div>
            ...
            {showPlatformPicker && <PlatformPickerModal connectedIds={connectedIds} connecting=
            {connecting} onClose={() => setShowPlatformPicker(false)} onConnect={handleConnect}/>}
